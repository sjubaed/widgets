...
function Multiselection(props: AllWidgetProps<any>) {
  ...
  const changeHandlerFBDC = () => {
    if (props.useDataSources.length > 0) {
      ...
      if (enableDC) { var targetVal2 = "DC" }
      else { var targetVal2 = "NONE" }

      const queryParams: SqlQueryParams = {
        where: (`${'USER_Type'} LIKE '%${targetVal}%' OR ${'USER_Type'} LIKE '%${targetVal2}%'`)
      };
  ...
  useEffect(() => {
    if (showFBDCGroup || enableFB) {
    changeHandlerFBDC()}
  }, [enableFB])
  useEffect(() => {
    if (showFBDCGroup || enableDC) {
      changeHandlerFBDC()}
  }, [enableDC])
      ...
      <Collapse in={collapseOpen}>
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
          ...
          <Accordion onChange={FBDCGroup} className={`MuiAccordion`}>
            <AccordionSummary>
              <Typography align="center" sx={{ fontSize: 13 }}>Food Bank Council of Michigan</Typography>
            </AccordionSummary>
...
{/*              <ToggleButton className={`MuiToggleButton ${enableFB ? "Mui-selected" : "MuiToggleButton"}`} value="FB" onChange={ShowFB} sx={{textTransform: "none"}}>Food Banks</ToggleButton>
            ...
            <FormGroup>
              <FormControlLabel checked={enableFB} className={`MuiToggleButton ${enableFB ? "Mui-selected" : "MuiToggleButton"}`} control={<Checkbox value="FB" onChange={ShowFB} checkedIcon={<DoneRoundedIcon style={{ color: '#00274C', fontSize: 'medium'}}/>} icon={<CloseRoundedIcon style={{ fontSize: 'medium'}}/>} />} label={<Typography sx={{ fontSize: 13 }}>Food Banks</Typography>} labelPlacement="end"/>
              <FormControlLabel checked={enableDC} className={`MuiToggleButton ${enableDC ? "Mui-selected" : "MuiToggleButton"}`} control={<Checkbox value="DC" onChange={ShowDC} checkedIcon={<DoneRoundedIcon style={{ color: '#00274C', fontSize: 'medium'}}/>} icon={<CloseRoundedIcon style={{ fontSize: 'medium'}}/>} />} label={<Typography sx={{ fontSize: 13 }}>Distribution Centers</Typography>} labelPlacement="end"/>
            </FormGroup>
