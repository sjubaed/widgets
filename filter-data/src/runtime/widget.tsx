/** @jsx jsx */
import { React, AllWidgetProps, jsx, DataSourceComponent, SqlQueryParams, DataSourceManager, QueriableDataSource, DataSource, FeatureLayerDataSource } from 'jimu-core'
...
export default class Widget extends React.PureComponent<AllWidgetProps<unknown>, State> {
  ...
  getFilter = (FB_Type: FBtype): string => {
    if (FB_Type && FB_Type !== FBtype.All) {
      return `(USER_Type = '${FB_Type}')`
    }
    return '(1=1)'
  }

  onRadioButtonChange = e => {
    const FBType = e.target.value
    ...
    if (dataSource) {
      ...
      dataSource.updateQueryParams(this.getQuery(FBType), this.props.id)
    }
  }
  ...
  render () {
    return (
      <div className='widget-demo jimu-widget m-2'>
        ...
        />
        <div role='radiogroup' aria-label={"Choose"}>
          <b>{this.props.intl.formatMessage({ id: 'selectType', defaultMessage: 'Filter by:'})}</b><br />
          <Label style={{ cursor: 'pointer' }} className='d-flex align-items-center'>
            <Radio
              style={{ cursor: 'pointer' }} value={FBtype.All} checked={this.state.FB_Type === FBtype.All} onChange={(evt) => {
          ...
          <Label style={{ cursor: 'pointer' }} className='d-flex align-items-center'>
            <Radio
              style={{ cursor: 'pointer' }} value={FBtype.FB} checked={this.state.FB_Type === FBtype.FB} onChange={(evt) => {
                this.onRadioButtonChange(evt);
                }}
          ...
          <Label style={{ cursor: 'pointer' }} className='d-flex align-items-center'>
            <Radio
              style={{ cursor: 'pointer' }} value={FBtype.DC} checked={this.state.FB_Type === FBtype.DC} onChange={(evt) => {
                this.onRadioButtonChange(evt);
                }}
